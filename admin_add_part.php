...
<head>
    ...
    <style>
        ...
        .form-container { max-width: 800px; margin: 40px auto; background-color: #fff; border-radius: 12px; padding: 30px; box-shadow: 0 0 20px rgba(0,0,0,0.08); }
        h2 { font-weight: bold; color: #2c3e50; border-left: 6px solid #3498db; padding-left: 15px; margin-bottom: 25px; }
        label.form-label { font-weight: bold; color: #34495e; }
...
<div class="form-container">
    ...
    <form id="estimateForm" method="POST">
        ...
        <div class="mb-3">
            <label class="form-label">📋 維修項目明細</label>
  ...
  <div class="modal-dialog">
    <div class="modal-content p-3">
      ...
      <div class="modal-body">
        ...
        <p><strong>估價內容：</strong><br><pre id="previewItems"></pre></p>
...
function updatePlates() {
    ...
    plateSelect.innerHTML = plates.length === 0 ? '<option value="">此會員尚無車輛</option>' : '<option value="">請選擇車牌</option>';
...
function calculateSubtotal(input) {
    ...
    row.querySelector('[name="item_total[]"]').value = subtotal.toFixed(0);
    updateTotal();
}
...
function confirmSubmit() {
    ...
    const specs = document.getElementsByName('item_spec[]');
    const qtys = document.getElementsByName('item_qty[]');
    const prices = document.getElementsByName('item_price[]');
    const notes = document.getElementsByName('item_note[]');

    for (let i = 0; i < names.length; i++) {
        const name = names[i].value.trim();
        const spec = specs[i].value.trim();
        const qty = qtys[i].value;
        const price = prices[i].value;
        const note = notes[i].value.trim();
        if (name || spec || qty || price || note) {
            content += `${name}（${spec}） x${qty} 單價 ${price}`;
            if (note) content += ` ｜ 備註：${note}`;
            content += "\n";
        }
    }
    document.getElementById('items').value = content.trim();
    previewEstimate();
}
